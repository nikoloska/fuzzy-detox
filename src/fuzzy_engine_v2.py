...
# ------------------------------------------------------------
# IMPROVEMENT 2 — Adjust LateNightUse "high" membership function
# Source: Exelmans & Van den Bulck (2016), "Bedtime mobile phone use and sleep
#         in adults", Social Science & Medicine, 148, 93-101.
#         The study found that the transition into clearly harmful late-night
#         phone use occurs beyond ~80 min of bedtime screen exposure, not at 65 min
#         as previously set. Shifting the left shoulder of the "high" trapezoid
#         from [65, 90] to [80, 105] prevents moderate use (~65-80 min) from
#         being over-penalised. Also corroborated by:
#         Exelmans & Van den Bulck (2020), PLOS ONE — restricting phone use 80 min
#         before bed yielded significantly improved sleep duration (+21 min).
# ------------------------------------------------------------
...
focus_rules = [
    ...
    # ------------------------------------------------------------
    # IMPROVEMENT 3 — Separate IdleChecking weight / soften standalone rule
    # Source: Siebers, Beyens & Valkenburg (2024), "The effects of fragmented
    #         and sticky smartphone use on distraction and task delay",
    #         Mobile Media & Communication, 12(1).
    #         Frequent checking behaviour causes attentional switching costs
    #         (Mark et al., 2005), but the "very_low" FocusQuality outcome is
    #         disproportionate when IdleChecking is the *only* elevated input.
    #         Very_low is reserved for the combined case of high IdleChecking AND
    #         high ScreenGlances (both fragmentation sources co-occurring), which
    #         research identifies as maximally disruptive.
    # Also: Ward et al. (2017), "Brain Drain", Journal of the Association for
    #        Consumer Research — even the presence of a phone reduces available
    #        cognitive capacity, but the effect compounds with co-occurring cues.
    # ------------------------------------------------------------
    ctrl.Rule(idle_checking["high"], focus_quality["low"]),                        # standalone: low (not very_low)
...
digital_overload_rules = [
    ...
    # ------------------------------------------------------------
    # IMPROVEMENT 1 — Tighten rule base: social media high but other inputs moderate/low
    # Source: Mehta, R.N.A. (2023), "Applying Fuzzy Logic for Multicriteria
    #         Performance Analysis of Social Media Networking", International
    #         Journal of Fuzzy Computation and Modelling, 5(3).
    #         The paper demonstrates that a single elevated antecedent (social
    #         media) in a MIMO fuzzy system should not trigger the maximum
    #         consequent when co-occurring inputs are low-to-moderate. A combined
    #         antecedent rule constraining the consequent to Medium provides a
    #         more accurate inference. Replaced the former single-antecedent
    #         "high" rule with a compound rule that requires at least one
    #         additional elevated input to reach "high" overload, and adds a
    #         new "medium" rule for the isolated high-social-media case.
    # ------------------------------------------------------------
