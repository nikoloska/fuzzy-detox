...
/* WHAT-IF */
.whatif-box { padding: 0.8rem 1rem; border-radius: 10px; background: rgba(16,185,129,0.05); border: 1px solid rgba(16,185,129,0.15); margin-bottom: 0.5rem; }
.whatif-label { font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; color: rgba(16,185,129,0.6); letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 0.3rem; }
.whatif-delta { font-family: 'Outfit', sans-serif; font-size: 1.4rem; font-weight: 700; }
.whatif-desc  { font-family: 'Space Grotesk', sans-serif; font-size: 0.8rem; color: rgba(226,232,240,0.5); }
...
with st.sidebar:
    ...
    # What-If simulator
    st.markdown('<div class="sidebar-brand" style="margin-top:0">◈ what-if simulator</div>', unsafe_allow_html=True)
    st.caption("Reduce one variable and see the predicted impact.")
    whatif_var = st.selectbox("Reduce variable", ["LateNightUse","ScreenGlances","IdleChecking","SocialMediaUsage"], label_visibility="collapsed")
    whatif_by  = st.slider("Reduce by", 5, 50, 20, 5, label_visibility="collapsed")
...
# What-if calculation
wi_vals = dict(vals)
wi_vals[whatif_var] = max(0, wi_vals[whatif_var] - whatif_by)
wi_result = evaluate_fuzzy_system(
    screen_glances_value=wi_vals["ScreenGlances"],
    idle_checking_value=wi_vals["IdleChecking"],
    late_night_use_value=wi_vals["LateNightUse"],
    social_media_value=wi_vals["SocialMediaUsage"],
)
wi_habit = wi_result["outputs"]["HabitBalance"]
wi_delta = wi_habit - habit
...
<div class="hero-wrapper">
    <div class="hero-eyebrow">Mamdani Fuzzy Inference System · Two-Subsystem Architecture</div>
    <div class="hero-title">Digital Habit <span>Intelligence</span></div>
    <div class="hero-sub">Real-time fuzzy analysis of your phone behaviour — not how much you use it, but how well.</div>
...
with c1:
    ...
    wi_col  = "#34d399" if wi_delta > 0.1 else "#f87171" if wi_delta < -0.1 else "#fbbf24"
    st.markdown(f'''
    <div class="card card-glow-cyan">
        ...
        <div style="margin-top:0.8rem;padding:0.6rem 0.8rem;border-radius:8px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05)">
            <div style="font-family:JetBrains Mono,monospace;font-size:0.6rem;letter-spacing:0.12em;text-transform:uppercase;color:rgba(16,185,129,0.5);margin-bottom:0.3rem">What-if: reduce {whatif_var} by {whatif_by}</div>
            <span style="font-family:Outfit,sans-serif;font-size:1.1rem;font-weight:700;color:{wi_col}">{wi_sign}{wi_delta:.2f}</span>
            <span style="font-family:JetBrains Mono,monospace;font-size:0.75rem;color:rgba(226,232,240,0.3)"> → {wi_habit:.2f}/10</span>
        </div>
    </div>
    ''', unsafe_allow_html=True)
...
with r3c:
    ...
    if ic >= 40: rules_fired.append(("focus","IF IdleChecking IS High → FocusQuality IS Very Low","#f87171"))
    ...
    # Sleep rules
    if ln >= 65 and sm >= 55: rules_fired.append(("sleep","IF LateNightUse IS High AND SocialMedia IS High → SleepQuality IS Low","#f87171"))
    if ln >= 65 and sg >= 65: rules_fired.append(("sleep","IF LateNightUse IS High AND ScreenGlances IS High → SleepQuality IS Low","#f87171"))
...
```
All membership functions are grounded in psychology literature: Kushlev (2017), Rasch & Born (2013), Combertaldi et al. — Fribourg (2021).
""")
